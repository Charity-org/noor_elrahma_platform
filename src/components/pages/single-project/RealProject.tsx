...
import { singleProjectData } from "@/constants/singleProjectData";
...
function RealProject() {
  ...
  const tweenParallax = useCallback((emblaApi: EmblaCarouselType) => {
    ...
    scrollSnapList.forEach((scrollSnap, snapIndex) => {
      ...
      slidesInSnap.forEach((slideIndex) => {
        const translate = diffToTarget * (-1 * 35) + "%";
        const layer = slides[slideIndex].querySelector(".real-project-embla__parallax__layer");
  ...
  return (
    <section className="container mt-12 md:mt-24 mb-16">
      ...
      <div className="real-project-embla" ref={emblaRef}>
        <div className="real-project-embla__container">
          {singleProjectData.map(({ image, id }, index) => (
            <div key={`${id}-${index}`} className="real-project-embla__slide">
              <div className="real-project-embla__parallax">
                <div className="real-project-embla__parallax__layer">
                  <Image
                    src={image}
                    alt={`Project moment ${id}`}
                    fill
                    className="real-project-embla__parallax__img"
